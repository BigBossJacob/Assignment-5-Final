...
local audio1 = audio.loadSound( "Assets/8bit.m4a" )
audio.play(audio1)
...
local bleach = display.newImageRect("Assets/bleach.png", 40, 50)
bleach.x = math.random (1, 320)
bleach.y = -110 
bleach.id = "bleach"
physics.addBody( bleach, "static", { 
    friction = 0, 
    bounce = 0 
    } )
...
local function MoveImage4(event)
    bleach.y = bleach.y + scrollSpeed3
end

local function imageReset ( event )
    if bleach.y >= 600 then
...
if orange.y >= 600 or apple.y >= 600 or banana.y >= 600 or catch == 1 then
        ...
        if catch == 1 then
            prompt.text = "You caught" 
            prompt2.text = "the bleach!"
        else   
...
bleach.collision = bleachCollision
bleach:addEventListener( "collision" )
--TEST FUNCTIONS
